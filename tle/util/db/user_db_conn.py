...
class UserDbConn:
    ...
    def create_tables(self):
        ...
        # Internal Points System for ICPC Mentorship
        self.conn.execute('''
            CREATE TABLE IF NOT EXISTS internal_points (
                ...
                last_solved_count INTEGER DEFAULT 0
            )
        ''')
    ...
    # Mentorship System Helpers
    def update_user_points(self, user_id, weekly_delta=0, monthly_delta=0, total_delta=0, last_solved_count=None):
        query_sel = 'SELECT weekly_points, monthly_points, total_points, last_solved_count FROM internal_points WHERE user_id = ?'
        res = self.conn.execute(query_sel, (user_id,)).fetchone()
        
        if res:
            w, m, t, l = res
            new_w = w + weekly_delta
            new_m = m + monthly_delta
            new_t = t + total_delta
            new_l = last_solved_count if last_solved_count is not None else l
            query_upd = '''
                UPDATE internal_points 
                SET weekly_points = ?, monthly_points = ?, total_points = ?, last_solved_count = ?
                WHERE user_id = ?
            '''
            self.conn.execute(query_upd, (new_w, new_m, new_t, new_l, user_id))
        else:
            new_w = weekly_delta
            new_m = monthly_delta
            new_t = total_delta
            new_l = last_solved_count if last_solved_count is not None else 0
            query_ins = '''
                INSERT INTO internal_points (user_id, weekly_points, monthly_points, total_points, last_solved_count)
                VALUES (?, ?, ?, ?, ?)
            '''
            self.conn.execute(query_ins, (user_id, new_w, new_m, new_t, new_l))
        
        self.conn.commit()
        self.update()

    def get_user_points(self, user_id):
        query = 'SELECT weekly_points, monthly_points, total_points, last_solved_count FROM internal_points WHERE user_id = ?'
        return self.conn.execute(query, (user_id,)).fetchone()

    def reset_weekly_points(self):
        self.conn.execute('UPDATE internal_points SET weekly_points = 0')
        self.conn.commit()
        self.update()

    def reset_monthly_points(self):
        self.conn.execute('UPDATE internal_points SET monthly_points = 0')
        self.conn.commit()
        self.update()

    def get_all_points(self):
        query = 'SELECT user_id, weekly_points, monthly_points, total_points FROM internal_points'
        return self.conn.execute(query).fetchall()
